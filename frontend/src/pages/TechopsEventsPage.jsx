...
export default function TechopsEventsPage() {
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          {!showDashboard ? (
                ...
                {events.filter(event => !event.event_finished).map(event => (
                  <div key={event._id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                      <h3 style={{ fontSize: '1.3rem', fontWeight: '600', margin: '0', color: 'var(--text-primary)' }}>
                        {event.title}
                      </h3>
                      {isEventToday(event.date) && (
                        <span style={{
                          background: 'linear-gradient(135deg, #4ade80, #22c55e)',
                          color: 'white', padding: '0.3rem 0.6rem', borderRadius: '12px',
                          ...
                          {event.hr.allocated_volunteers?.length > 0 && (
                            <div style={{ marginLeft: '1rem', fontSize: '0.8rem', marginTop: '0.25rem' }}>
                              {event.hr.allocated_volunteers.map((vol, idx) => (
                                <div key={idx}>• {vol.volunteer_name} - {vol.volunteer_role} ({vol.volunteer_contact})</div>
                              ))}
                            </div>
                          )}
                        </div>
            ...
            <>
              ...
              <TechopsDashboard onBackToEvents={() => setShowDashboard(false)} />
            </>
          )}
        </div>
      </div>
